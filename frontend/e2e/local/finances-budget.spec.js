...
test('crea movimientos reales y un presupuesto mensual local', async ({ page }) => {
  ...
  await page.goto('/finances/recurring')
  await page.getByRole('button', { name: 'Nueva regla' }).click()
  dialog = page.getByRole('dialog', { name: 'Crear regla recurrente' })
  await dialog.getByLabel('Monto').fill('12000')
  await dialog.getByLabel('Cuenta').selectOption({ label: `${accountName} - Corriente` })
  await dialog.getByRole('button', { name: 'Gasto' }).click()
  await dialog.getByLabel(/Categor/).selectOption({ label: categoryName })
  await dialog.getByLabel(/Descripci/).fill('Regla E2E')
  await dialog.getByLabel('Frecuencia').selectOption('daily')
  await dialog.getByRole('button', { name: 'Crear regla' }).click()
  await expect(page.getByRole('button', { name: 'Registrar ahora' }).first()).toBeVisible()
  await page.getByRole('button', { name: 'Registrar ahora' }).first().click()
  dialog = page.getByRole('dialog', { name: 'Registrar movimiento' })
  await dialog.getByLabel(/Descripci/).fill('Registro recurrente E2E')
  ...
  await page.goto('/finances/movements')
  await expect(page.getByText('Registro recurrente E2E', { exact: true })).toBeVisible()
